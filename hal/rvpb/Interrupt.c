...
void Hal_register_handler(InterHdlr_fptr handler, uint32_t interrupt_num)
{
    sHandlers[interrupt_num] = handler;
}
void Hal_interrupt_run_handler(void)
{
    // get acknowledged interruptID
    uint32_t interrupt_num = GicCpu->interruptack.bits.InterruptID;

    // call handler for interruptID
    if (sHandlers[interrupt_num] != NULL)
    {
        sHandlers[interrupt_num]();
    }

    // notice handling for interruptID is finished
